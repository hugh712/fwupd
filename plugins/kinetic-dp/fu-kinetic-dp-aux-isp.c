...
static const KtDpChipBrIdStrTable kt_dp_branch_dev_info_table[] = {
    ...
    /* Puma MC29x0 */
    {KT_CHIP_PUMA_2900, KT_FW_STATE_RUN_IROM, {'P', 'U', 'M', 'A'},4},
...
const gchar *
fu_kinetic_dp_aux_isp_get_fw_run_state_str(
                                                                                                                     KtFwRunState fw_run_state)
{
  return (fw_run_state < KT_FW_STATE_NUM) ? 
                                                              kt_dp_fw_run_state_strs[fw_run_state] : NULL;
}

static gboolean
fu_kinetic_dp_aux_isp_get_basic_dev_info_from_branch_id(
                                                                  const guint8 *br_id_str_buf,
                                                                  ...
                                                                  GError **error)
{
  ...
  *   DPCD branch ID string  (when length is 6) */
  ...
  g_set_error(error,FWUPD_ERROR,FWUPD_ERROR_INTERNAL,
                               "%s is not a supported Kinetic device",str);
  return FALSE;
}

gboolean
fu_kinetic_dp_aux_isp_enable_aux_forward(
                                                    ...
                                                    GError **error)
{
  if (root_dev_state != KT_FW_STATE_RUN_APP) {
    ...
    if (root_dev_chip_id == KT_CHIP_JAGUAR_5000 || 
      ...
      if (!fu_kinetic_dp_secure_aux_isp_enable_aux_forward(connection, target_port,
                                                                                                                                                          error)){
...
gboolean
fu_kinetic_dp_aux_isp_disable_aux_forward(
                                                        FuKineticDpConnection *connection,
                                                        KtChipId root_dev_chip_id,
                                                        KtFwRunState root_dev_state,
                                                        GError **error)
...
gboolean
fu_kinetic_dp_aux_isp_read_basic_device_info(
                                                                                                                          ...
                                                                                                                          GError **error)
{
  ...
  if (!fu_kinetic_dp_aux_dpcd_read_branch_id_str(connection,
                               dev_info_local.branch_id_str,sizeof(dev_info_local.branch_id_str),error)){
    return FALSE;
  }
   if (!fu_kinetic_dp_aux_isp_get_basic_dev_info_from_branch_id(
        dev_info_local.branch_id_str,sizeof(dev_info_local.branch_id_str),
         &dev_info_local,error)){
    return FALSE;
   }
...
{
    	g_debug("aux isp class initalized.");
} 
