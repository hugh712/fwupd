...
G_DEFINE_TYPE_WITH_PRIVATE(FuKineticDpSecureAuxIsp,
                                                  fu_kinetic_dp_secure_aux_isp,FU_TYPE_KINETIC_DP_AUX_ISP)
...
#define DPCD_ADDR_KT_AUX_WIN_END       (DPCD_ADDR_KT_AUX_WIN +                           \
                                                                                          DPCD_SIZE_KT_AUX_WIN - 1)
...
#define STD_FW_SIGNATURE_OFFSET                                                                                                 \
                                    (STD_FW_PAYLOAD_SIZE - STD_APP_ID_SIZE + 4)  /* 0xFFFE4 */
#define STD_FW_VER_OFFSET                                                                                                                 \
                                    (STD_FW_PAYLOAD_SIZE - STD_APP_ID_SIZE + 12) /* 0xFFFEC */
#define STD_FW_VER_SIZE                           3
#define CUSTOMER_PROJ_ID_OFFSET                                                                                                 \
                                     (STD_FW_PAYLOAD_SIZE - STD_APP_ID_SIZE + 15) /* 0xFFFEF */
#define CUSTOMER_FW_VER_OFFSET                                                                                                  \
                                    (STD_FW_PAYLOAD_SIZE - STD_APP_ID_SIZE + 16) /* 0xFFFF0 */
...
#define SPI_APP_CERTIFICATE_START                                                                                           \
...
#define SPI_APP_PAYLOAD_START                                                                                                   \
                    (SPI_ESM_PAYLOAD_START +  ESM_PAYLOAD_BLOCK_SIZE)           /*0x41000*/
...
static gboolean
fu_kinetic_dp_secure_aux_isp_read_param_reg(
                                                    FuKineticDpConnection *self,guint8 *dpcd_val,GError **error)
{

  if (!fu_kinetic_dp_connection_read(self, DPCD_ADDR_PARAM_REG, 
                                                                         dpcd_val, 1, error)){
    g_prefix_error(error, "failed to read DPCD_KT_PARAM_REG: ");
    return FALSE;
  }
  //g_debug("reading param_reg: 0x%x",*dpcd_val);
  return TRUE;
}

static gboolean
fu_kinetic_dp_secure_aux_isp_write_kt_prop_cmd(
                                                     FuKineticDpConnection *connection,
                                                     guint8 cmd_id,
                                                     GError **error)
{
  //g_debug("sending cmd 0x%x",cmd_id);
  cmd_id |= DPCD_KT_CONFIRMATION_BIT;

  if (!fu_kinetic_dp_connection_write(connection, 
                                                                          DPCD_ADDR_CMD_STATUS_REG, 
                                                                          &cmd_id, sizeof(cmd_id),error)){
...
static gboolean
fu_kinetic_dp_secure_aux_isp_clear_kt_prop_cmd(
                                                                          FuKineticDpConnection *connection, GError **error)
{
  guint8 cmd_id = KT_DPCD_CMD_STS_NONE;

  //g_debug("clear cmd reg");
  if (!fu_kinetic_dp_connection_write(connection,
                                                                          DPCD_ADDR_CMD_STATUS_REG,
                                                                          &cmd_id,sizeof(cmd_id),error)){
    g_prefix_error(error, "failed to write DPCD_KT_CMD_STATUS_REG: ");
...
static gboolean
fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(
                                                                                                                  FuKineticDpConnection *self,
                                                                                                                  guint8 cmd_id,
                                                                                                                  guint32 max_time_ms,
                                                                                                                  ...
                                                                                                                  GError **error)
{
  ...
  *status = KT_DPCD_CMD_STS_NONE;
  ...
  //g_debug("waiting for cmd 0x%x processing...",cmd_id);
  /* Wait for the sent proprietary command to be processed */
  while (max_time_ms != 0) {
    if (!fu_kinetic_dp_connection_read(self,DPCD_ADDR_CMD_STATUS_REG,
        ...
        *status = dpcd_val & DPCD_KT_COMMAND_MASK;
        ...
        if (KT_DPCD_STS_CRC_FAILURE == *status) {
          ...
          g_set_error(error,FWUPD_ERROR,FWUPD_ERROR_INTERNAL,
                                    "invalid replied value in DPCD_KT_CMD_STATUS_REG: 0x%X",*status);
        }
        return FALSE;
        } else { /* dpcd_val == cmd_id */
        /* confirmation bit is cleared by sink,
         * means that sent command is processed */
        //g_debug("cmd 0%x processed",cmd_id);
        return TRUE;
      }
    }
...
static gboolean
fu_kinetic_dp_secure_aux_isp_read_dpcd_reply_data_reg(
                                                            FuKineticDpConnection *self,
                                                            guint8 *buf,
                                                            ...
                                                            GError **error)
{
  ...
  /* set the output to 0 */
  *read_len = 0;
  //g_debug("reading from reply_data reg");
  if (!fu_kinetic_dp_connection_read(self,DPCD_ADDR_ISP_REPLY_LEN_REG,
  ...
  if (buf_size < read_data_len) {
    g_set_error(error,FWUPD_ERROR,FWUPD_ERROR_INTERNAL,
                            "buffer size [%u] is not enough to read DPCD_ISP_REPLY_DATA_REG [%u]",
                              buf_size,read_data_len);
    return FALSE;
  }

  if (read_data_len > 0) {
    if (!fu_kinetic_dp_connection_read(self,DPCD_ADDR_ISP_REPLY_DATA_REG,
                                                                           buf, read_data_len,error)) {
      g_prefix_error(error, "failed to read DPCD_ISP_REPLY_DATA_REG: ");
      return FALSE;
    }
    *read_len = read_data_len;
    //g_debug("%u bytes read",read_data_len);
  }
...
static gboolean
fu_kinetic_dp_secure_aux_isp_write_dpcd_reply_data_reg(
                           FuKineticDpConnection *self, const guint8 *buf, guint8 len,GError **error)
{
  gboolean ret = FALSE;
  gboolean res;

//g_debug("write to reply_data reg");
  if (len > DPCD_SIZE_ISP_REPLY_DATA_REG) {
    g_set_error(error, FWUPD_ERROR,FWUPD_ERROR_INTERNAL,
                            "length bigger than DPCD_SIZE_ISP_REPLY_DATA_REG [%u]",len);
    return FALSE;
  }

  res = fu_kinetic_dp_connection_write(self ,DPCD_ADDR_ISP_REPLY_DATA_REG,
                                                                                buf, len, error);
  if (!res) {
  ...
  if (fu_kinetic_dp_connection_write(self, DPCD_ADDR_ISP_REPLY_LEN_REG,
                                                           &len, DPCD_SIZE_ISP_REPLY_LEN_REG, error) &&res) {
    /* both reply data and reply length are written successfully */
   //g_debug("wrote %u bytes",len);
    ret = TRUE;
  }
  return ret;
}
...
static gboolean
fu_kinetic_dp_secure_aux_isp_write_mca_oui(
                                                      FuKineticDpConnection *connection, GError **error)
{
  guint8 mca_oui[DPCD_SIZE_IEEE_OUI] = {MCA_OUI_BYTE_0, 
                                                                                     MCA_OUI_BYTE_1, 
                                                                                     MCA_OUI_BYTE_2};
  //g_debug("write mca OUI");
  return fu_kinetic_dp_aux_dpcd_write_oui(connection, mca_oui, error);
}

static gboolean
fu_kinetic_dp_secure_aux_isp_enter_code_loading_mode(
                                                          FuKineticDpConnection *self,
  ...
  if (is_app_mode) {
    ...
    * to make DPCD 514writable */
    if (!fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(self,
                                        KT_DPCD_CMD_PREPARE_FOR_ISP_MODE,500,10, &status,error)){
      return FALSE;
    }
  }
  /* Update payload size to DPCD reply data reg first */
  if (!fu_kinetic_dp_secure_aux_isp_write_dpcd_reply_data_reg(self,
                                                                                      (guint8 *)&code_size,sizeof(code_size),error)){
    return FALSE;
  }

  if (!fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(self,
                                  KT_DPCD_CMD_ENTER_CODE_LOADING_MODE,500,10,&status,error)){
    return FALSE;
    }
  return TRUE;
}
...
static gboolean
fu_kinetic_dp_secure_aux_isp_send_payload(
                                                                                                        FuKineticDpSecureAuxIsp *self,
                                                                                                        ...
                                                                                                        GError **error)
{
  ...
  while (remain_payload_len > 0) {
    chunk_len = (remain_payload_len >= DPCD_SIZE_KT_AUX_WIN) ? 
                                                                                  DPCD_SIZE_KT_AUX_WIN: remain_payload_len;
    ...
    if (!fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(connection,
                                                                                  ...
                                                                                  wait_time_ms,wait_interval_ms,&status, error)) {
...
static gboolean
fu_kinetic_dp_secure_aux_isp_wait_dpcd_cmd_cleared(
                                                          FuKineticDpConnection *self,
...
static gboolean
fu_kinetic_dp_secure_aux_isp_execute_isp_drv(
                                                        FuKineticDpSecureAuxIsp *self,
                                                        FuKineticDpConnection *connection,
                                                        GError **error)
{
  ...
  if (!fu_kinetic_dp_secure_aux_isp_write_kt_prop_cmd(connection,
                                                                             KT_DPCD_CMD_EXECUTE_RAM_CODE,error)){
    return FALSE;
   }

  if (!fu_kinetic_dp_secure_aux_isp_wait_dpcd_cmd_cleared(connection,1500,100,
                                                                                                                         &status,error)) {
  ...
  if (KT_DPCD_STS_SECURE_ENABLED == status) {
    ...
    priv->isp_total_size -= (FW_CERTIFICATE_SIZE * 2 + 
                                                  FW_RSA_SIGNATURE_BLOCK_SIZE * 2);
  }

  if (!fu_kinetic_dp_secure_aux_isp_read_dpcd_reply_data_reg(connection, reply_data, 
                                                                                                                              sizeof(reply_data),
                                                                                                                              &read_len,error)) {
  ...
  if (!fu_common_read_uint16_safe(reply_data,sizeof(reply_data),0,&priv->flash_id,
                                                        G_BIG_ENDIAN,error)){
    return FALSE;
  }
  if (!fu_common_read_uint16_safe(reply_data,sizeof(reply_data),2,&priv->flash_size,
                                                           G_BIG_ENDIAN,error)){
    return FALSE;
   }
  if (!fu_common_read_uint16_safe(reply_data,sizeof(reply_data),4,
                                                                      &priv->read_flash_prog_time,G_BIG_ENDIAN,error)){
    return FALSE;
  }
...
static gboolean
fu_kinetic_dp_secure_aux_isp_send_isp_drv(
                                                                  FuKineticDpSecureAuxIsp *self,
                                                                  FuKineticDpDevice *device,
                                                                  FuKineticDpConnection *connection,
                                                                  gboolean is_app_mode,
                                                                  ...
                                                                  GError **error)
{
  ...
  if (!fu_kinetic_dp_secure_aux_isp_send_payload(self, device,  connection,  isp_drv_data,
                                                                                                      isp_drv_len,  progress,  10000,  50, 
                                                                                                      error)) {
...
static gboolean
fu_kinetic_dp_secure_aux_isp_enable_fw_update_mode(
                                                        ...
                                                        GError **error)
{
  ...
  if (!fu_kinetic_dp_secure_aux_isp_write_dpcd_reply_data_reg(connection, pl_size_data, 
                                                                                                                          sizeof(pl_size_data), error)) {
    g_prefix_error(error, "send payload size failed: ");
    return FALSE;
  }

  if (!fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(connection,
          KT_DPCD_CMD_ENTER_FW_UPDATE_MODE,200000,500,&status, error)) {
...
static gboolean
fu_kinetic_dp_secure_aux_isp_send_app_fw(
                                        FuKineticDpSecureAuxIsp *self,
                                        FuKineticDpDevice *device,
                                        FuKineticDpConnection *connection,
                                        FuKineticDpFirmware *firmware,
                                        ...
                                        GError **error)
{
  ...
  if (priv->is_isp_secure_auth_mode) {
    ...
    if (!fu_kinetic_dp_secure_aux_isp_send_payload(self,device,connection, ptr,
                                         FW_CERTIFICATE_SIZE * 2 +FW_RSA_SIGNATURE_BLOCK_SIZE * 2,
                                         fu_progress_get_child(progress),10000, 200,error)) {
  ...
  ptr = (guint8 *)fw_data + 
            (fu_kinetic_dp_firmware_get_is_fw_esm_xip_enabled(firmware)? 
            SPI_APP_EXTEND_INIT_DATA_START : SPI_APP_NORMAL_INIT_DATA_START);
  if (!fu_kinetic_dp_secure_aux_isp_send_payload(self,device,connection,ptr,
                                                            fu_kinetic_dp_firmware_get_app_init_data_size(firmware),
                                                            fu_progress_get_child(progress),10000,200,error)) {
  ...
  if (!fu_kinetic_dp_secure_aux_isp_send_payload(self, device, connection, ptr,
                                STD_APP_ID_SIZE,  fu_progress_get_child(progress),10000, 200, error)) {
...
static gboolean
fu_kinetic_dp_secure_aux_isp_install_fw_images(
                                                  FuKineticDpSecureAuxIsp *self,
                                                  FuKineticDpDevice *device,
                                                  FuKineticDpConnection *connection,
                                                  GError **error)
{
  ...
  while (wait_count-- > 0) {
    if (!fu_kinetic_dp_connection_read(connection, DPCD_ADDR_CMD_STATUS_REG,
                                                                                                                                            &status, 1,  error)) {
...
static void
fu_kinetic_dp_secure_aux_isp_send_reset_command(
                                                    FuKineticDpConnection *connection)
{
  g_autoptr(GError) error_local = NULL;
  g_debug("sending reset...");
  if (!fu_kinetic_dp_secure_aux_isp_write_kt_prop_cmd(connection,
                                        KT_DPCD_CMD_RESET_SYSTEM,  &error_local)){
    g_warning("failed to reset system: %s", error_local->message);
  }
}

static KtFlashBankIdx
fu_kinetic_dp_secure_aux_isp_get_flash_bank_idx(
                                        FuKineticDpConnection *connection, GError **error)
{
  guint8 status;
  ...
  if (!fu_kinetic_dp_aux_dpcd_read_oui(connection, prev_src_oui, 
                                                                              sizeof(prev_src_oui), error)){
    return BANK_NONE;
  }
  ...
  if (fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(connection,
                                KT_DPCD_CMD_GET_ACTIVE_FLASH_BANK,100,20,&status, error) &&
...
gboolean
fu_kinetic_dp_secure_aux_isp_enable_aux_forward(
                                                          FuKineticDpConnection *connection,
                                                          ...
                                                          GError **error)
{
  ...
  if (!fu_kinetic_dp_connection_write(connection,DPCD_ADDR_PARAM_REG,
                                                                          &cmd_id,sizeof(cmd_id),error)){
    return FALSE;
  }
  if (!fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(connection,
                                         KT_DPCD_CMD_ENABLE_AUX_FORWARD,  1000,20,&status, error)){
    return FALSE;
  }
  /* Clear CMD_STATUS_REG */
  cmd_id = KT_DPCD_CMD_STS_NONE;
  return fu_kinetic_dp_connection_write(connection,DPCD_ADDR_CMD_STATUS_REG, 
                                                                                 &cmd_id,sizeof(cmd_id),error);
}

gboolean
fu_kinetic_dp_secure_aux_isp_disable_aux_forward(
FuKineticDpConnection *connection, GError **error)
{
  gboolean ret;
  guint8 status;
  ...
  ret = fu_kinetic_dp_secure_aux_isp_send_kt_prop_cmd(connection,
                                      KT_DPCD_CMD_DISABLE_AUX_FORWARD,  1000,  20, &status,error);

  /* clear CMD_STATUS_REG, it's a necessary step after sending a proprieatry command */
  cmd_id = KT_DPCD_CMD_STS_NONE;
  fu_kinetic_dp_connection_write(connection, DPCD_ADDR_CMD_STATUS_REG,
                                                                   &cmd_id,sizeof(cmd_id),error);
  return ret;
}

static gboolean
fu_kinetic_dp_secure_aux_isp_get_device_info(
  ...
  g_debug("secure aux read device info(hw_ver, fw_ver, customer_id, customer_fw_ver, chip_id)");
  connection = fu_kinetic_dp_connection_new(fu_udev_device_get_fd(FU_UDEV_DEVICE
                                                                                                                                                              (device)));

  /* chip ID, FW work state, and branch ID string are known */
  if (!fu_kinetic_dp_connection_read(connection,DPCD_ADDR_BRANCH_HW_REV,
                                                                        dpcd_buf,sizeof(dpcd_buf),  error)){
    return FALSE;
    }
  /* DPCD 0x509 hw_ver */
  dev_info->chip_rev = dpcd_buf[0];                       
  /* DPCD 0x50A ~ 0x50C fw_ver */
  dev_info->fw_info.std_fw_ver = (guint32)dpcd_buf[1] << 16 |
                                                                  (guint32)dpcd_buf[2] << 8 |
                                                                  dpcd_buf[3];               
  /* DPCD 0x515  customer_proj_id */
  dev_info->fw_info.customer_project_id = dpcd_buf[12];
  /* DPCD (0x50F | 0x514) customer_fw_ver*/
  dev_info->fw_info.customer_fw_ver =(guint16)dpcd_buf[6] << 8 | 
                                                                              (guint16)dpcd_buf[11];
  /* DPCD 0x516 */
                                    ...
                                    GError **error)
{
  ...
  connection = fu_kinetic_dp_connection_new(
  ...
  if (payload_len > 0) {
    if (!fu_kinetic_dp_secure_aux_isp_send_isp_drv(sec_aux_isp,device, connection,
                                                                                                is_app_mode, payload_data,payload_len,
  ...
  if (!fu_kinetic_dp_secure_aux_isp_enable_fw_update_mode(firmware_self, connection, 
                                                                                                                          error)){
    goto SECURE_AUX_ISP_END;
  }
  /* get image of App FW */
  ...
  if (!fu_kinetic_dp_secure_aux_isp_send_app_fw(sec_aux_isp,device, connection, 
                                                                                                firmware_self,payload_data,  payload_len,
                                                                                                fu_progress_get_child(progress),error)){
    goto SECURE_AUX_ISP_END;
  }
  fu_progress_step_done(progress); 
...
gboolean
fu_kinetic_dp_secure_aux_isp_parse_app_fw(
                                                                  FuKineticDpFirmware *firmware,
                                                                  ...
                                                                  GError **error)
{
  ...
  memcpy(&customer_fw_ver, &fw_bin_buf[CUSTOMER_FW_VER_OFFSET], 
                                                                                                                           CUSTOMER_FW_VER_SIZE);
  fu_kinetic_dp_firmware_set_customer_fw_ver(firmware, customer_fw_ver);

  /* get each block size from FW buffer */
  fu_kinetic_dp_firmware_set_esm_payload_size(firmware,
                        fu_kinetic_dp_firmware_get_valid_payload_size(
                              &fw_bin_buf[SPI_ESM_PAYLOAD_START], ESM_PAYLOAD_BLOCK_SIZE));
  fu_kinetic_dp_firmware_set_arm_app_code_size(firmware,
                        fu_kinetic_dp_firmware_get_valid_payload_size(
                              &fw_bin_buf[SPI_APP_PAYLOAD_START], app_code_block_size));
  fu_kinetic_dp_firmware_set_app_init_data_size(firmware,
                        fu_kinetic_dp_firmware_get_valid_payload_size(
                              &fw_bin_buf[app_init_data_start_addr],APP_INIT_DATA_BLOCK_SIZE));
  fu_kinetic_dp_firmware_set_cmdb_block_size(firmware,
                        fu_kinetic_dp_firmware_get_valid_payload_size(
                                &fw_bin_buf[SPI_CMDB_BLOCK_START],CMDB_BLOCK_SIZE));
...
static void
fu_kinetic_dp_secure_aux_isp_class_init(
                                        FuKineticDpSecureAuxIspClass *klass)
{
